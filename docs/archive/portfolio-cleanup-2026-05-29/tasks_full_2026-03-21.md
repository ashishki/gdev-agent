...
## Portfolio Maintenance Phase

Status: planned
Purpose: keep the project interview-ready without expanding product scope.
...
Scope:
Review README, demo instructions, architecture diagram references, and setup
commands so the project presents cleanly to technical interviewers.

Acceptance Criteria:
1. README explains the product, architecture, setup, and demo path without
   internal development-process noise.
2. Demo instructions run against documented local prerequisites.
3. No new product features are opened by this task.
...
Scope:
Run the current smoke/test commands and record any breakage as maintenance
findings.

Acceptance Criteria:
1. Test/lint command results are recorded in the project docs or issue notes.
2. Any failure is triaged as setup drift, dependency drift, or code regression.
3. No unrelated refactor is made during smoke verification.
...
Scope:
Keep dependency/security posture reasonable for a portfolio project.

Acceptance Criteria:
1. Dependency updates are made only when needed for installability or security.
2. Changes are tested with the documented smoke path.
3. Public-facing docs remain focused on product/engineering value.
